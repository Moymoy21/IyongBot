      ...
      try {
        ...
        if (interaction.isChatInputCommand()) {
          ...
          if (selectMenu) {
            ...
            catch (error) { await handleInteractionError(interaction, error, withTraceContext({ type: 'select_menu', customId: interaction.customId }, interactionTraceContext)); }
          }
        } else if (interaction.isModalSubmit()) {
          
          // --- PET MODAL SYSTEM START ---
          if (interaction.customId.startsWith('submit_listing_')) {
            const modal = client.modals.get('submit_listing');
            if (modal) {
              try {
                await modal.execute(interaction, client);
                return;
              } catch (error) {
                logger.error('Error in submit_listing modal:', error);
              }
            }
          }
          ...
          if (interaction.customId.startsWith('pet-modal-')) {
            try {
              ...
              return;
            } catch (error) { logger.error('Error in pet-modal submission:', error); }
          }
          // --- PET MODAL SYSTEM END ---

          if (interaction.customId.startsWith('app_modal_')) {
            try { await handleApplicationModal(interaction); }
            catch (error) { await handleInteractionError(interaction, error, withTraceContext({ type: 'modal', customId: interaction.customId, handler: 'application' }, interactionTraceContext)); }
            return;
          }

          if (interaction.customId.startsWith('app_review_')) {
            try { await handleApplicationReviewModal(interaction); }
            catch (error) { await handleInteractionError(interaction, error, withTraceContext({ type: 'modal', customId: interaction.customId, handler: 'application_review' }, interactionTraceContext)); }
            return;
          }
          ...
          if (modal) {
            ...
            catch (error) { await handleInteractionError(interaction, error, withTraceContext({ type: 'modal', customId: interaction.customId, handler: 'general' }, interactionTraceContext)); }
          }
        }
      } catch (error) {
